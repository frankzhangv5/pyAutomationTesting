...
class BeautifulReport(ReportTestResult):
    ...
    def __init__(self, suites, report_dir=os.path.join(os.getcwd(), "result")):
        super(BeautifulReport, self).__init__(suites)
        self.suites = suites
        self.report_dir = report_dir
        self.title = '自动化测试报告'
        self.filename = 'report.html'
        self.img_path = os.path.join(report_dir, "Screenshots")
    ...
    def report(self, description, filename: str = None):
        ...
        self.output_report()
        text = '\n测试已全部完成, 可前往查询测试报告: {}{}{}'.format(self.report_dir, os.sep, self.filename)
        print(text)
    ...
    def output_report(self):
        ...
        with TemporaryFile('w+t', encoding='utf-8') as f:
            ...
            body = f.readlines()
            f.close()
        with open(override_path + self.filename, 'wt', encoding='utf-8') as write_file:
            for item in body:
                if item.strip().startswith('var resultData'):
                    head = '    var resultData = '
                    item = item.split(head)
    ...
    def img2base(img_path: str, file_name: str) -> str:
        ...
        with open(os.path.join(img_path, file_name), 'rb') as file:
            data = file.read()
